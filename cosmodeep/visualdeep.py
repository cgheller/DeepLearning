...
def getActivations(layer,stimuli,nx,ny):
    layername = "conv1"
    nnn = nx*ny
    units = sess.run(layer,feed_dict={x:np.reshape(stimuli,[1,nnn],order='F'),keep_prob:1.0})
    dims = np.ma.shape(units)
    nelements = dims[3]
    
    for ii in range(nelements):
       filename = layername+"-"+str(ii)+".jpg"
       f = open(filename, "wb+")
       imageaux = units[:,0:dims[1],0:dims[2],ii]
       xmax = np.amax(imageaux)
       xmin = np.amin(imageaux)
       xnorm = 255/xmax
       datainput256 = tf.multiply(imageaux, xnorm)
       datainputuintaux = tf.cast(datainput256, tf.uint8)
       datainputuint = tf.reshape(datainputuintaux,[dims[1],dims[2],Nin])
...
writelayer = 0
checkpoint = 0
checkpointfile = './model-noisetot.ckpt'
restart = 1
restartfile = './model-noisetot.ckpt'
...
if readdata == 1:
   ...
   path = '/scratch/snx3000/cgheller/CosmoDeep/results/noise-tot/'
   print("Reading: ")
...
#sess = tf.Session(config=tf.ConfigProto(intra_op_parallelism_threads=12, log_device_placement=True))
sess = tf.Session()
sess.run(init)
if restart == 1:
 ...
 #for i in range(iend-istart):
  for i in range(50):
   if actualdata[istart+i,0] == 1:
     filename = "noise"+str(i)+".jpg"
   if actualdata[istart+i,1] == 1:
     filename = "tot"+str(i)+".jpg"
...
if writelayer == 1:
   imagetoshow = 14
   imageToUse = imagedata[istart+imagetoshow,:]
   #plt.imshow(np.reshape(imageToUse,[imagex,imagey]), interpolation="nearest", cmap="gray")
   getActivations(h_conv1,imageToUse,imagex,imagey)
...
accuracy_time = time.time() - acc_time
training_time = end_train_time - train_time
reading_time = train_time - read_time
print("test accuracy %g"%test_acc)

elapsed_time = time.time() - start_time

print(' ')
print('read time (sec)     = ', reading_time)
print('training time (sec) = ', training_time)
print('test time (sec)     = ', accuracy_time)
